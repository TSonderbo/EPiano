...
@njit
def sim(freq = 440, over = 32, len = 1, damping = False, fsOrg = 44100):
    ...
    fs = 44100 * overFs #sample rate
    k = 1/fs #sampling Period
    kSq = k**2
    
    lenSound = math.floor(fsOrg * len) #length of sound in samples
    ...
    sigma_0 = 0.0005 #freq. independent damping
    sigma_1 = 0.001 #freq. dependent damping
    ...
    f_vIn = 3 #Initial velocity (m/s) typical range [1, 4] - Would be mapped to mpe-midi input
    f_u = -1*10**-4 #Current hammer position
    f_uPrev = f_u - k*f_vIn #Set initial velocity
    f_uNext = 0
    f_ratio = f_m / (rho*A*L)
    f_contact = math.floor(M_u*0.8)
    x = 0 #Compression
...
#output, f_out, x_out, d_force = sim(len=5, freq=261.63)
# plt.plot(x_out, f_out)
# plt.show()

# e = np.arange(len(output))*1/(48000)
